...
def post_job():
    ...
    if t in ['src', 'text', 'raw']:
        sentences = sent_tokenize(str(body))
    else:
        sentences = copy(body)
...
class Worker(Process):
    ...
    def send_request(self, sock, text, olang, ostyle):
    ...
    def translate(self, params_str, requests, sock):
        ...
        while True:
            with requests as r:
                ...
                responses = self.send_request(sock, text, olang, ostyle)
    ...
    def wait_connection(self, sock, check_every=60):
        p = current_process()
        # sock.settimeout(2)
        while True:
            try:
